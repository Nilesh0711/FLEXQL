...
\title{\textbf{FlexQL Database Engine}\\\Large Detailed Design and Architecture Document}
...
flowchart TD
    ...
    subgraph DiskLayout
        K[Table Metadata<br/>data/tables/*.meta]
        L[Row Files<br/>data/tables/*.rows]
        M[Index Files<br/>data/indexes/*.idx]
    end
    ...
    classDef clients fill:#fdecc8,stroke:#c08b2c,color:#2b2112,stroke-width:1.5px;
    classDef access fill:#dbeafe,stroke:#3b82f6,color:#122033,stroke-width:1.5px;
    classDef core fill:#e9d5ff,stroke:#8b5cf6,color:#24123a,stroke-width:1.5px;
    classDef runtime fill:#dcfce7,stroke:#22c55e,color:#163221,stroke-width:1.5px;
    classDef disk fill:#ffe4e6,stroke:#e11d48,color:#3a1420,stroke-width:1.5px;
...
%%}}}%%
flowchart TD
    A[Client issues SQL] --> B[Send length-prefixed SQL]
    B --> C[Worker thread receives request]
    C --> D{Statement type}
    D -->|SELECT| E[Acquire read lock]
    D -->|CREATE / INSERT / DELETE / RESET| F[Acquire write lock]
    E --> G[Parse SQL]
    F --> G
    G --> H[Executor dispatch]

    H --> I{Execution path}
    I -->|Read path| J[Check query cache]
    J --> K{Cache hit?}
    K -->|Yes| L[Replay cached rows]
    K -->|No| M[Run scan, index lookup, or join]

    I -->|Write path| N[Append SQL to WAL]
    N --> O[Execute storage and index update]
    O --> P[Invalidate cache]
    P --> Q[Clear WAL]

    L --> R[Stream results to callback]
    M --> R
    Q --> R

    classDef client fill:#fdecc8,stroke:#c08b2c,color:#2b2112,stroke-width:1.5px;
    classDef control fill:#dbeafe,stroke:#2563eb,color:#10243e,stroke-width:1.5px;
    classDef readpath fill:#dcfce7,stroke:#16a34a,color:#163221,stroke-width:1.5px;
    classDef writepath fill:#ffe4e6,stroke:#e11d48,color:#3a1420,stroke-width:1.5px;
    classDef decision fill:#ede9fe,stroke:#7c3aed,color:#24123a,stroke-width:1.5px;

    class A,B,C client;
    class E,F,G,H,R control;
    class J,K,L,M readpath;
    class N,O,P,Q writepath;
    class D,I decision;
\end{lstlisting}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{diagrams/index_structure.png}
...
%%}}}%%
flowchart TD
    R[Root / Internal Page<br/>keys: 100, 250]

    subgraph LeafLevel
        direction TB
        L1[Leaf Page A<br/>keys: 1..99<br/>row offsets]
        L2[Leaf Page B<br/>keys: 100..249<br/>row offsets]
        L3[Leaf Page C<br/>keys: 250+<br/>row offsets]
    end
    ...
    classDef root fill:#ddd6fe,stroke:#7c3aed,color:#24123a,stroke-width:1.5px;
    classDef leaf fill:#dcfce7,stroke:#16a34a,color:#163221,stroke-width:1.5px;
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{diagrams/wal_recovery.png}
...
%%}}}%%
flowchart TD
    A[Server start] --> B[Initialize storage catalog and cache]
    B --> C[Open data/wal/flexql.wal]
    C --> D{WAL empty?}
    D -->|Yes| E[Continue startup]
    D -->|No| F[Read next SQL record]
    F --> G[Re-execute statement]
    G --> H{More WAL records?}
    H -->|Yes| F
    H -->|No| I[Clear WAL]
    I --> E

    classDef startup fill:#dbeafe,stroke:#2563eb,color:#10243e,stroke-width:1.5px;
    classDef wal fill:#ffe4e6,stroke:#e11d48,color:#3a1420,stroke-width:1.5px;
    classDef decision fill:#ede9fe,stroke:#7c3aed,color:#24123a,stroke-width:1.5px;
    classDef done fill:#dcfce7,stroke:#16a34a,color:#163221,stroke-width:1.5px;
...
flowchart TD
    ...
    classDef clients fill:#fdecc8,stroke:#c08b2c,color:#2b2112,stroke-width:1.5px;
    classDef workers fill:#dbeafe,stroke:#2563eb,color:#10243e,stroke-width:1.5px;
    classDef locks fill:#ede9fe,stroke:#7c3aed,color:#24123a,stroke-width:1.5px;
    classDef ops fill:#dcfce7,stroke:#16a34a,color:#163221,stroke-width:1.5px;
    classDef writes fill:#ffe4e6,stroke:#e11d48,color:#3a1420,stroke-width:1.5px;

    class A,B,C clients;
    class TA,TB,TC workers;
    class R,W locks;
    class S ops;
    class M writes;
\end{lstlisting}
